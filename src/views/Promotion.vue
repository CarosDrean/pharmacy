          ...
          <div class="shadow overflow-hidden border-b border-gray-200 sm:rounded-lg">
            <table class="min-w-full divide-y divide-gray-200">
              ...
              <tr v-for="promotion in promotions" :key="promotion.id">
                ...
                <td class="px-6 py-4 whitespace-nowrap">
                  <div class="text-sm text-gray-900">{{ promotion.percentage }}</div>
                </td>
                <td class="px-6 py-4 whitespace-nowrap">
                  <span
                        ...
                        <div class="px-4 py-5 bg-white sm:p-6">
                          <div class="grid grid-cols-6 gap-6">
                            <div class="col-span-6">
                              ...
                              <input type="text" name="description" id="description" autocomplete="name" v-model="promotion.description"
                                     class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/>
                            </div>

                            <div class="col-span-6">
                              <label for="percentage" class="block text-sm font-medium text-gray-700">Porcentaje</label>
                              <input type="number" name="percentage" id="percentage" autocomplete="price" v-model="promotion.percentage"
                                     class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/>
                            </div>

                            <div class="col-span-6">
                              <label for="start" class="block text-sm font-medium text-gray-700">Fecha de Inicio</label>
                              <input type="date" name="start" id="start" autocomplete="location"
                                     v-model="promotion.start_date"
                                     class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/>
                            </div>

                            <div class="col-span-6">
                              <label for="finish" class="block text-sm font-medium text-gray-700">Fecha de Fin</label>
                              <input type="date" name="finish" id="finish" autocomplete="location"
                                     v-model="promotion.finish_date"
                                     class="mt-1 focus:ring-indigo-500 focus:border-indigo-500 block w-full shadow-sm sm:text-sm border-gray-300 rounded-md"/>
                            </div>
                          </div>
                        </div>
...
export default {
  ...
  methods: {
    async getPromotions() {
      axios.get(`${process.env.VUE_APP_API}/v1/promotion`)
          .then(response => {
